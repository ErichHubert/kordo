...
`kordo.run.created` event to Hatchet. A separate Hatchet worker process marks
the run running, calls the runner, materializes artifacts, and finalizes the run.

The control plane API does not expose a workflow callback endpoint. Hatchet
workers connect to the Hatchet engine and register Kordo workflows from the
...
- Local development needs the Hatchet engine and a Kordo Hatchet worker.
- Hatchet provides the outer workflow engine for the later PI multi-role flow.
- The API process and worker process now have a clearer deployment boundary.
